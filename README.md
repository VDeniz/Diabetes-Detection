...
1. Clone the repository:
   ```bash
   git clone https://github.com/VDeniz/Diabetes-Detection.git
   ```
...
- **Dataset**: The dataset used for analysis is available on Google Drive: [Download diabetes_binary_health_indicators_BRFSS2015.csv](https://drive.google.com/file/d/1preqqvZgSZUkM8hGcoBockhX-yEjVhuV/view?usp=sharing). Place it in `C:/Diabetes_Detection/`.
...
**Note**: Ensure the dataset file (`diabetes_binary_health_indicators_BRFSS2015.csv`) is downloaded from the provided Google Drive link and placed in `C:/Diabetes_Detection/` before running the script. Verify that the output directory exists and has write permissions.

**Note**: Ensure the output directory (`C:/Diabetes_Detection/`) exists and has write permissions before running the script. For large datasets, ensure sufficient system resources (e.g., at least 8 GB RAM) to avoid memory issues.